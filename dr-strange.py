...
def draw_magic_circle(frame, cx, cy, radius, t, intensity=1.0):
    ...
    for i in range(8):
        ...
        pts_oct.append([x, y])
    cv2.polylines(frame, [np.array(pts_oct, np.int32)], True, tuple(int(c * alpha) for c in bright_gold), 2,
                  cv2.LINE_AA)
    ...
    for offset in [0, math.pi / 4]:
        ...
        for i in range(4):
            ...
            pts_sq.append([x, y])
        cv2.polylines(frame, [np.array(pts_sq, np.int32)], True, tuple(int(c * alpha) for c in bright_gold), 2,
                      cv2.LINE_AA)
    ...
    cv2.circle(frame, (cx, cy), int(radius * 0.05 + 5 * pulse), tuple(int(c * alpha) for c in white_gold), -1,
               cv2.LINE_AA)
...
def main():
    ...
    while cap.isOpened():
        ...
        if left_fist_present:
            if not portal_active:
                if right_hand_landmarks:
                    ...
                    is_circle, center, avg_radius = check_circle_gesture(drawing_points, min_perimeter=250,
                                                                         close_threshold=50)
                    if is_circle:
                        portal_active = True
        ...
        if portal_active or portal_intensity_val > 0.05:
            draw_magic_circle(blend, int(portal_center[0]), int(portal_center[1]), portal_radius, t,
                              portal_intensity_val)
